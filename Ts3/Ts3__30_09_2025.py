...
x_ruidosa = x_clean + ruido  # Señales con ruido agregado

# ----------------------------
# APLICACIÓN DE VENTANAS
# ----------------------------
xw = x_ruidosa[:, None, :, :] * W[None, :, None, :]  # Aplicar cada ventana a cada señal
...
# ----------------------------
# --- Estimador de amplitud en Ω0 con ganancia de ventana ---
k0 = int(np.round(Ω0 * N / (2*np.pi)))  # Índice fijo correspondiente a Ω0
G_w = np.sum(W, axis=1) / N  # Ganancia coherente de cada ventana
G_w_safe = np.where(G_w == 0, 1.0, G_w)

# Extraer magnitud en Ω0 de forma robusta
a_hats_at_Omega0 = np.take_along_axis(Xmag, np.full(Xmag.shape[:3] + (1,), k0), axis=3)[...,0]

# Corregir por ganancia de ventana
a_hats = a_hats_at_Omega0 / G_w_safe[None, :, None]
