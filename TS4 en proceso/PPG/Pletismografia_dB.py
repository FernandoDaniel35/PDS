...
# -*- coding: utf-8 -*-
""" 
Densidad Espectral de Potencia de la señal de Pletismografía - TODOS LOS GRÁFICOS SEPARADOS E INDIVIDUALES.
...
# Tomar la primera columna de datos
...
print(f"Usando {N} muestras de Pletismografía para el análisis.")
...
plt.plot(t, pletismografia, linewidth=1.5)
plt.title("1. Señal de Pletismografía en el Dominio del Tiempo")
plt.xlabel("Tiempo [s]")
plt.ylabel("Amplitud [unidades arbitrarias]")
plt.grid(True)
plt.tight_layout()
plt.savefig('resultados/1_Señal_Pletismografia_Tiempo.png', dpi=300)
plt.show()
plt.close()
...
# ----------------------------
Y_fft = np.fft.fft(pletismografia)
P_fft = np.abs(Y_fft)
...
plt.title("2. Espectro de Magnitud (FFT) de Pletismografía")
plt.xlabel("Frecuencia [Hz]")
plt.ylabel("Magnitud (Lineal)")
plt.xlim(0, 50) 
plt.grid(True)
plt.tight_layout()
plt.savefig('resultados/2_FFT_Pletismografia_Magnitud.png', dpi=300)
plt.show()
plt.close()
...
f_per, Pxx_per_lin = periodogram(pletismografia, fs=fs, window=ventana, nfft=N, scaling='density')
...
# --- CÁLCULO DE WELCH ---
start_time = time.time()
f_w, Pxx_w_lin = welch(pletismografia, fs=fs, window='hamming', nperseg=2048, noverlap=1024, scaling='density')
...
start_time = time.time()
M = 1024 
f_bt, Pxx_bt_lin = per_smooth(pletismografia, fs=fs, M=M)
