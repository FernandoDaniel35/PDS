...
# -*- coding: utf-8 -*-
"""
Densidad Espectral de Potencia del ECG - TODOS LOS GRÁFICOS SEPARADOS E INDIVIDUALES.

*** CÓDIGO FINAL COMPLETO: CORREGIDO (KeyError), EN dBV²/Hz, Y CON 9 GRÁFICOS INDIVIDUALES ***
- Todos los gráficos son separados.
- DEP en Decibelios de Potencia (10 * log10(Pxx)).
- Incluye Señal en Tiempo, FFT, 3 Estimadores, 3 BW separados, 1 Comparativa.
"""
...
# ----------------------------
# Cargar archivo .mat y parámetros
# ----------------------------
try:
    mat_contents = sio.loadmat('ECG_TP4.mat')
except FileNotFoundError:
    print("Error: Asegúrate de tener el archivo 'ECG_TP4.mat' en el mismo directorio.")
    exit()

# *** CORRECCIÓN DEL KeyError: 'ecg' ***
ecg_full = np.squeeze(mat_contents['ecg_lead']) 
fs = 1000  # Frecuencia de muestreo en Hz

# ----------------------------
# Ajustar cantidad de muestras para cálculo
# ----------------------------
N_samples = 15000  # 15 segundos de datos a 1000 Hz
ecg = ecg_full[:N_samples]
N = len(ecg)
t = np.arange(N) / fs # Vector de tiempo
print(f"Usando {N} muestras del ECG para el análisis.")
...
# ----------------------------
# I.1. Señal ECG en el Tiempo
# ----------------------------
plt.figure(figsize=(10, 3))
plt.plot(t, ecg, linewidth=1.5)
plt.title("1. Señal ECG en el Dominio del Tiempo")
plt.xlabel("Tiempo [s]")
plt.ylabel("Amplitud [unidades arbitrarias]")
plt.grid(True)
plt.tight_layout()
plt.savefig('resultados/1_Señal_ECG_Tiempo.png', dpi=300)
plt.show()
plt.close()
...
# ----------------------------
Y_fft = np.fft.fft(ecg)
P_fft = np.abs(Y_fft)
f_fft = np.fft.fftfreq(N, 1/fs)

half_N = N // 2
plt.figure(figsize=(10, 4))
plt.plot(f_fft[:half_N], P_fft[:half_N], linewidth=1.5)
plt.title("2. Espectro de Magnitud (FFT) de la Señal ECG")
plt.xlabel("Frecuencia [Hz]")
plt.ylabel("Magnitud (Lineal)")
plt.xlim(0, 50) 
plt.grid(True)
plt.tight_layout()
plt.savefig('resultados/2_FFT_ECG_Magnitud.png', dpi=300)
plt.show()
plt.close()
...
f_per, Pxx_per_lin = periodogram(ecg, fs=fs, window=ventana, nfft=N, scaling='density')
...
# --- CÁLCULO DE WELCH ---
start_time = time.time()
f_w, Pxx_w_lin = welch(ecg, fs=fs, window='hamming', nperseg=2048, noverlap=1024, scaling='density')
...
start_time = time.time()
M = 1024 
f_bt, Pxx_bt_lin = per_smooth(ecg, fs=fs, M=M)
