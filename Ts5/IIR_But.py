...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 11 02:00:43 2025

@author: fer
"""
# -*- coding: utf-8 -*-
"""
Filtro_IIR_Butterworth_Final.py - IIR (Máxima Planicidad)
...
# Función para graficar Polos y Ceros (Consigna c)
...
# 2. DISEÑO IIR BUTTERWORTH (Consigna c)
sos = sig.iirdesign(wp, ws, gpass=alpha_p, gstop=alpha_s, ftype='butter', output='sos', fs=fs)
# Aplicación con Fase Cero
ECG_f = sosfiltfilt(sos, ecg_one_lead) 

z, p, k = sig.sos2zpk(sos)
zplane(z, p, title=f'Polos y Ceros - Butterworth IIR (N={len(sos) * 2})')

# 3. EVALUACIÓN CONSOLIDADA (Consigna d)

# A. Evaluación de Atenuación (Ruido: 3 Señales en la misma figura)
regs_ruido = ([4000, 5500], [10000, 11000]) # Regiones con ruido 
for ii in regs_ruido:
    ...
    plt.plot(zoom_region, ECG_f[zoom_region], label='3. ECG Filtrado (Butterworth)', linewidth=2, color='blue')
    plt.title(f'Evaluación de Atenuación: Región con Ruido {ii[0]}-{ii[1]}'); plt.legend(); plt.grid(True)
    plt.tight_layout(); plt.show()

# B. Evaluación de Inocuidad (Sin Ruido: 3 Señales Limpio vs. Ruido vs. Filtrado)
