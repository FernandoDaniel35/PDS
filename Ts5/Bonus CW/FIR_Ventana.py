# -*- coding: utf-8 -*-
"""
Created on Tue Nov 11 13:24:16 2025
...
# -*- coding: utf-8 -*-
"""
Filtro_FIR_Ventana_CW.py - FIR (Pasa Banda)
Diseño: Método de Ventanas (Hamming)
...
fs = 44100
# Orden alto (N=3000) necesario para un filtro pasa banda estrecho
...
# 2. DISEÑO FIR VENTANA (Band-Pass)
f_deseada = [0, ws[0], wp[0], wp[1], ws[1], fs / 2]
...
zplane(z, p, title=f'Polos y Ceros - FIR Ventana Pasa Banda (N={N_fir})')
...
# Esto es necesario para que la señal filtrada esté alineada temporalmente con la original.
zoom_region = np.arange(0, cant_muestras - demora, dtype='uint')
plt.figure(figsize=(12, 6))
# Gráfico de la señal original (hasta el punto final compensado)
plt.plot(zoom_region / fs, audio_one_lead[zoom_region], label='1. Audio Original (CW + Ruido)', color='red', alpha=0.6)
# Gráfico de la señal filtrada (inicio en el retardo, hasta el final)
plt.plot(zoom_region / fs, Audio_f[zoom_region + demora], label=f'2. Audio Filtrado (Ventana - τg={demora} mues)', linewidth=2, color='blue')
